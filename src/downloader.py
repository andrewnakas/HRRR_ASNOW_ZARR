"""
NOMADS HRRR Data Downloader
Downloads ASNOW variable from NOMADS using grib filter for bandwidth efficiency
"""
...
class NomadsDownloader:
    """Download HRRR ASNOW data from NOMADS with rate limiting"""
    ...
    def __init__(self, config_path="config.yaml"):
        ...
        self.base_url = self.config['nomads']['base_url']
    ...
    def download_asnow_filtered(self, date_hour, output_dir="tmp"):
        """
        ...
        # Output filename
        filename = output_path / f"hrrr_asnow_{date_str}_{hour_str}.grib2"
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                logger.info(f"Downloading {date_hour} (attempt {attempt + 1}/{self.retry_attempts})")
                ...
                if response.status_code == 200:
                    # Check if we actually got GRIB data (not an error page)
                    content_type = response.headers.get('Content-Type', '')

                    if 'text/html' in content_type:
                        logger.warning(f"Received HTML instead of GRIB2 for {date_hour}")
                    ...
                    # Verify file size (ASNOW-only should be ~6-8 MB)
                    ...
                    logger.info(f"✓ Downloaded {filename.name} ({file_size_mb:.2f} MB)")
                    return filename
    ...
    def download_date_range(self, start_date, end_date, output_dir="tmp"):
        """
        Download ASNOW data for a date range
        ...
        while current <= end_date:
            filepath = self.download_asnow_filtered(current, output_dir)
        ...
        logger.info(f"Downloaded {len(downloaded_files)} files for date range")
        return downloaded_files


def download_single_file(date_str, hour, output_dir="tmp", config_path="config.yaml"):
    """
    ...
    """
    downloader = NomadsDownloader(config_path)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python downloader.py YYYY-MM-DD HH [output_dir]")
        print("Example: python downloader.py 2024-01-15 12 tmp")
        sys.exit(1)
    ...
    filepath = download_single_file(date_str, hour, output_dir)
